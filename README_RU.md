...
- **✨ Демо**: прогоняет всё по очереди — 6 waveforms, громкость
  (тихо/норма/громко) и маппинг (velocity/pitch/drums) — с пульсирующим
  индикатором и подписью шага. Прерывается кнопкой ■ Стоп
...
проверено на MacBook Air: из 1–20 реально стучат только 1–6 (разница в основном
в громкости), 7+ молчат — поэтому для контраста используй повторы и buzz,
пауза повторов `--rep-gap 20..500`, мастер-сила `--master 10..300`
(настоящая амплитуда актуатора 0.1..2.0 — в окне это слайдер «Сила»),
или файл `~/.typeclickrc` (строки вида `enter=2x2`, `master=150`, перечитывается наживую).
...
каком железе), пауза, слайдер мастер-силы 10–300 с живым превью,
превью вибрации при наведении на эффект, проверка паттернов,
**▶ Демо** (одиночный/двойной/тройной/buzz/мощный по очереди с подписью).
Общий `~/.typeclickrc` с CLI-версией.
```bash
make typebar-app
open TypeBar.app
```
...
(лучше всего заходят мелодии восьмыми), ноты падают,
